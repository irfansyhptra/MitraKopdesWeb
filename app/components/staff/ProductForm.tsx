...
function ProductEditor({ product }: { product: Product | null }) {
  ...
  return <form onSubmit={(event) => void submit(event)} className="staff-stack staff-product-form">
    ...
    <button className="staff-btn staff-save" type="submit" disabled={busy || categories.loading || !!categories.error}>{busy ? (files.length ? `Mengunggah ${files.length} gambar…` : 'Menyimpan…') : product ? 'Simpan Perubahan' : 'Simpan Produk'}</button>
  </form>;
}
